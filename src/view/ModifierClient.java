...
public class ModifierClient extends JDialog {
	 ...
	 * Create the dialog.
	 */
	public ModifierClient(PanelClient panelClient, Client client, int index) {
		...
		JButton btnAjouterClient = new JButton("Modifier");
		...
		textFieldmatricule.setText(client.getMatricule());
		textFieldraison.setText(client.getRaison());
		if (client.getType().equals("Physique")) {
			btnNewButton.setSelected(true);
		}
		textFieldfax.setText(client.getFax());
		textFieldNumFix.setText(client.getNumFix());
		textFieldnumportable.setText(client.getNumPortable());
		textFieldEmail.setText(client.getEmail());
		textFieldSiteweb.setText(client.getSiteWeb());
		if (!client.getEtat()) {
			rdbtnNewRadioButton_1.setSelected(true);
		}
		Address address = client.getAddress();
		textFieldnumeroroute.setText(address.getNumeroRue());
		textFieldlibelle.setText(address.getLibelle());
		comboBoxvile.setSelectedItem(address.getVille());
		textFieldcodepostal.setText(address.getCodePostal());
		textFieldgouvernourat.setText(address.getGouvernourat());
		textFieldpays.setText(address.getPays());
		updateaccounttable(client.getBankAccounts());
		fillcurrentrib(client);
		...
		btnAjouterClient.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!Saisie.checkmatricule(textFieldmatricule.getText().trim())) {
					JOptionPane.showMessageDialog(ModifierClient.this,
				...
				Client c = new Client(textFieldmatricule.getText().trim(),
						textFieldraison.getText().trim(),
						buttonGroup.getSelection().getActionCommand(), bankAccounts, address,
						textFieldfax.getText().trim(),
						textFieldnumportable.getText().trim(), textFieldNumFix.getText().trim(), textFieldEmail.getText().trim(),
						textFieldSiteweb.getText().trim(), etat,
						client.getCode());
				clientService.edit(c);
	...
	public void updateaccounttable(ArrayList<BankAccount> list) {
